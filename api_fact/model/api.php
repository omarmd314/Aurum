...
class ApiModel
{
    ...
    public function generar_numero_de_nota($tipo_doc)
    {
        $consulta = $this->db->query("CALL sp_generar_numero_nota_credito('".$tipo_doc."',@numero)");
        $query    = $this->db->query('SELECT @numero AS numero');
        return $query->fetch(PDO::FETCH_OBJ);
    }

    public function buscar_comprobante($nro_doc)
    {
        $consulta = $this->db->prepare("CALL sp_consultar_documento_nota(?)");
        $consulta->bindValue(1, $nro_doc, PDO::PARAM_STR);
        $consulta->execute();
        return $consulta->fetch(PDO::FETCH_OBJ);
    }

    public function registrar_nota($cabecera)
    {
        $status = false;
        $rpta = new stdClass();
        ...
        try {
            ...
            $fieldNames = implode(', ', array_keys($cabecera));
            $fieldValues = "'" . implode("', '", $cabecera) . "'";
            $this->db->query("INSERT INTO nota_credito ($fieldNames) VALUES ($fieldValues)");
            $id_nota = $this->db->lastInsertId(); //Recuperamos el código del Resumen
            $status = true;
            //$this->db->query("UPDATE tm_venta SET estado='i' WHERE id_venta='".$cod_ven."'");
    ...
    public function actualizar_cdr_nota($dato)
    {
        $rpta = new stdClass();

        try {

            $stm = $this->db->prepare("UPDATE nota_credito SET enviado_sunat=?, hash_cpe=?,hash_cdr=?,code_respuesta_sunat=?,descripcion_sunat_cdr=?,name_file_sunat=? WHERE id_nota = ?");

            $stm->bindParam(1, $dato['enviado_sunat']);
            $stm->bindParam(2, $dato['hash_cpe']);
            $stm->bindParam(3, $dato['hash_cdr']);
            $stm->bindParam(4, $dato['code_respuesta_sunat']);
            $stm->bindParam(5, $dato['descripcion_sunat_cdr']);
            $stm->bindParam(6, $dato['name_file_sunat']);
            $stm->bindParam(7, $dato['id_nota']);
            $stm->execute();
