...
$total_bebidas=0;
 ...
 foreach ($this->dato->Detalle as $d) {
	 if (strval($d->cat) == '11') {
		 array_push($bebidas, $d);
	} else {
		
		array_push($comidas, $d);
	}
}
...
$pdf->Ln(8);
$pdf->SetFont('Courier','B',10);
$pdf->Cell(72,4,'== COMIDAS VENDIDAS ==',0,1,'C');
$pdf->Ln(1); 
// COLUMNAS
...
//=== COMIDAS ===
 ...
 foreach ($comidas as $c) {
	$pdf->SetFont('Courier', '', 8);
	$pdf->MultiCell(42,4,utf8_decode($c->pres),0,'L');

	$pdf->Cell(47, -4, $c->cantidad,0,0,'R');
	$pdf->Cell(10, -4, $c->precio,0,0,'R');
	$pdf->Cell(15, -4, number_format(($c->precio_t),2),0,0,'R');
	$pdf->Ln(1);

	$total_comidas += $c->precio_t;
} 
$pdf->Cell(72,0,'','T');
$pdf->Ln(1);

//=== TOTAL COMIDAS ===
$pdf->Cell(37, 4, 'TOTAL', 0);    
$pdf->Cell(20, 4, '', 0);
//$totalapp = $this->dato->Glovo->total + $this->dato->Rappi->total;
$pdf->Cell(15, 4, '= '.number_format($total_comidas,2),0,0,'R');
//===FIN TOTAL COMIDAS ===
...
$pdf->Ln(8);
$pdf->SetFont('Courier','B',10);
$pdf->Cell(72,4,'== BEBIDAS VENDIDAS ==',0,1,'C');
$pdf->Ln(1); 
// COLUMNAS
$pdf->SetFont('Courier', 'B', 8);
$pdf->Cell(42, 4, 'PRODUCTO', 0);
...
foreach ($bebidas as $b) {
	$pdf->SetFont('Courier', '', 8);
	$pdf->MultiCell(42,4,utf8_decode($b->pres),0,'L');

	$pdf->Cell(47, -4, $b->cantidad,0,0,'R');
	$pdf->Cell(10, -4, $b->precio,0,0,'R');
	$pdf->Cell(15, -4, number_format(($b->precio_t),2),0,0,'R');
	$pdf->Ln(1);

	$total_bebidas += $b->precio_t;
} 
$pdf->Cell(72,0,'','T');
$pdf->Ln(1);
...
$pdf->Cell(20, 4, '', 0);
//$totalapp = $this->dato->Glovo->total + $this->dato->Rappi->total;
$pdf->Cell(15, 4, '= '.number_format($total_bebidas,2),0,0,'R');
//===FIN TOTAL BEBIDAS ===

//FIN BEBIDAS VENDIDAS
