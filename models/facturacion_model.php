...
class Facturacion_Model extends Model
{
	...
	public function Datos1()
    {
        try
        {
            ...
            foreach($c as $k => $d)
            {
                $c[$k]->{'Cliente'} = $this->db->query("SELECT dni,ruc,nombre,direccion FROM v_clientes WHERE id_cliente = ".$d->id_cli)
                    ->fetch(PDO::FETCH_OBJ);
            }   
            $data = array("data" => $c);
    ...
    public function Datos6()
    {
        try
        {
            $ifecha = date('Y-m-d',strtotime($_POST['ifecha']));
            $ffecha = date('Y-m-d',strtotime($_POST['ffecha']));
            $stm = $this->db->prepare("SELECT * FROM nota_credito WHERE (fecha_nota >= ? AND fecha_nota <= ?)");
            $stm->execute(array($ifecha,$ffecha));
            $c = $stm->fetchAll(PDO::FETCH_OBJ); 
            $data = array("data" => $c);
            $json = json_encode($data);
