...
class Informe_Model extends Model
{
    ...
    public function finanza_arq_imp($data)
    {
        ...
        {      
            ...
            $c->{'PolloStock'} = $this->db->query("SELECT (ent-sal) AS total FROM v_stock WHERE id_tipo_ins = 1 AND id_ins = 1")->fetch(PDO::FETCH_OBJ);
            
            $c->{'Detalle'} = $this->db->query("SELECT dv.id_prod as 'id_prod',( select tp.id_catg from tm_producto tp,tm_producto_pres tpp  where tpp.id_pres=dv.id_prod and tpp.id_prod = tp.id_prod ) as 'cat',(select t.presentacion from tm_producto_pres t where t.id_pres = dv.id_prod) as 'pres', sum(dv.cantidad) as 'cantidad',dv.precio 'precio', sum(dv.cantidad)*dv.precio as 'precio_t' FROM tm_venta v,tm_detalle_venta dv   where v.id_apc={$data} and v.estado='a' and v.id_venta = dv.id_venta group by dv.id_prod order by 6  desc")->fetchAll(PDO::FETCH_OBJ);
