...
class Caja_Model extends Model
{
    ...
    public function egreso_list($data)
    {
        ...
        {   
            ...
            $id_usu = Session::get('usuid');
            $stm = $this->db->prepare("SELECT * FROM v_gastosadm WHERE DATE(fecha_re) >= ? AND DATE(fecha_re) <= ? AND  id_usu = ? AND id_tg LIKE ? AND estado like ?");
            $stm->execute(array($ifecha,$ffecha,$id_usu,$data['tipo_gasto'],$data['estado']));            
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            $data = array("data" => $c);
            $json = json_encode($data);
            echo $json; 
    ...
    public function egreso_crud_create($data)
    {
        try
        {
            ...
            $this->db->prepare($sql)->execute(array(
                $data['id_tipo_gasto'],intval($data['id_per']),$id_usu,$id_apc,$data['importe'],$data['responsable'],$data['motivo'],$fecha));
            $file = fopen("paso0039a.txt", "w");
            fwrite($file, "entro en INSERT INTO tm_gastos_adm(data " . PHP_EOL);            
            fwrite($file, json_encode(array($data['id_tipo_gasto'],intval($data['id_per']),$id_usu,$id_apc,$data['importe'],$data['responsable'],$data['motivo'],$fecha))."aca sql->" . PHP_EOL);
